...
Client.prototype.bindEventListeners = function() {
	...
	this.on("hi", function(msg) {
		self.user = msg.u;
		if (msg.secret) localStorage.secret = msg.secret;
...
Client.prototype.participantUpdate = function(update) {
	...
	if(part === null) {
		...
		if(update.admin) part.admin = update.admin;
		if(update.tag) part.tag = update.tag;
...
Client.prototype.startNote = function(note, vel, channel) {
	if(this.isConnected()) {
		var vel = typeof vel === "undefined" ? undefined : +vel.toFixed(3);
		if(!this.noteBufferTime) {
			this.noteBufferTime = Date.now();
			this.noteBuffer.push({n: note, v: vel, ch:channel});
		} else {
			this.noteBuffer.push({d: Date.now() - this.noteBufferTime, n: note, v: vel, ch:channel});
		}
	}
};
