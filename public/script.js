...
////////////////////////////////////////////////////////////////
	...
	function SoundSelector(piano) {
	    ...
	    this.addPack({name: "MPP Classic", keys: Object.keys(this.piano.keys), ext: ".mp3", url: "https://multiplayerpiano.net/sounds/mppclassic/"});
	}
...
////////////////////////////////////////////////////////////////
	...
	Piano.prototype.play = function(channel, note, vol, participant, delay_ms) {
		if(!this.keys.hasOwnProperty(note) || !participant) return;
		var key = this.keys[note];
		if(key.loaded && channel !== 9) this.audio.play(key.note, vol, delay_ms, participant.id);
		if(gMidiOutTest) gMidiOutTest(key.note, vol * 100, delay_ms, channel);
		var self = this;
		setTimeout(function() {
			if(channel !== 9) self.renderer.visualize(key, participant.color);
	...
	Piano.prototype.playDrum = function(note, vol, participant, delay_ms) {
		if(!this.keys.hasOwnProperty(note) || !participant) return;
		var key = this.keys[note];
		if(key.loaded) this.audio.play(key.note, vol, delay_ms, participant.id);
		if(gMidiOutTest) gMidiOutTest(key.note, vol * 100, delay_ms, 9);
		var self = this;
		setTimeout(function() {
			var jq_namediv = $(participant.nameDiv);
			jq_namediv.addClass("play");
	...
	Piano.prototype.stop = function(channel, note, participant, delay_ms) {
		if(!this.keys.hasOwnProperty(note)) return;
		var key = this.keys[note];
		if(key.loaded) this.audio.stop(key.note, delay_ms, participant.id);
		if(gMidiOutTest) gMidiOutTest(key.note, 0, delay_ms, channel);
	};
	
	var gPiano = new Piano(document.getElementById("piano"));
	
	var gSoundSelector = new SoundSelector(gPiano);
	gSoundSelector.addPacks(["https://multiplayerpiano.net/sounds/Emotional_2.0/", "https://multiplayerpiano.net/sounds/Harp/", "https://multiplayerpiano.net/sounds/Music_Box/", "https://multiplayerpiano.net/sounds/Vintage_Upright/", "https://multiplayerpiano.net/sounds/Steinway_Grand/", "https://multiplayerpiano.net/sounds/Emotional/", "https://multiplayerpiano.net/sounds/Untitled/","https://hri7566.github.io/SM64MusicBox/",
	...
	function press(channel, id, vol) {
		if(!gClient.preventsPlaying() && gNoteQuota.spend(1)) {
			gHeldNotes[id] = true;
			gSustainedNotes[id] = true;
			gPiano.play(channel, id, vol !== undefined ? vol : DEFAULT_VELOCITY, gClient.getOwnParticipant(), 0);
			gClient.startNote(id, vol, channel);
		}
	}

	function release(channel, id) {
		if(gHeldNotes[id]) {
			gHeldNotes[id] = false;
			if((gAutoSustain || gSustain) && !enableSynth) {
				...
				if(gNoteQuota.spend(1)) {
					gPiano.stop(channel, id, gClient.getOwnParticipant(), 0);
					gClient.stopNote(id);
...
////////////////////////////////////////////////////////////////
	...
	var gClient = new Client(protocol + "://" + (window.location.hostname));
	...
	gClient.on("disconnect", function(evt) {
		new Audio("https://files.catbox.moe/kluqgo.mp3").play();
		console.log(evt);
	});
	...
	(function() {
		...
		gClient.on("count", function(count) {
			if(count > 0) {
				...
				document.title = "Tinsoda Piano (" + count + ")";
			} else {
				document.title = "Tinsoda Piano";
			}
		});
	})();
  ...
  (function () {
    function setupParticipantDivs(part) {
      ...
      if (hadNameDiv) {
		...
		// nameDiv.addEventListener('mousedown', participantMenu(part))
        $(nameDiv).fadeIn(2000);
	...
	(function() {
		...
		gClient.on("cache-reload", msg => {
			location.reload(true)
		})
		gClient.on("hi", msg => {
			if(msg.u.admin == true) {
				$("#getcrown-btn").show(1)
				document.getElementById("getcrown-btn").addEventListener("click", () => {
					gClient.sendArray([{
						m:'get-crown'
					}])
				})
	...
	gClient.on("n", function(msg) {
		...
		for(var i = 0; i < msg.n.length; i++) {
			...
			if(note.s) {
				gPiano.stop(note.ch, note.n, participant, ms);
			} else {
				var vel = (typeof note.v !== "undefined")? parseFloat(note.v) : DEFAULT_VELOCITY;
				if(!vel) vel = 0;
				else if(vel < 0) vel = 0;
				else if (vel > 1) vel = 1;
				gPiano.play(note.ch, note.n, vel, participant, ms);
				if(enableSynth) {
					gPiano.stop(note.ch, note.n, participant, ms + 1000);
				}
			}
		}
	});
    ...
    participantTouchhandler = function (e, ele) {
      ...
      if (target_jq.hasClass("name")) {
        ...
        if (id == gClient.participantId) {
		  ...
		  if(gClient.user.admin !== true) $("#rename input[name=tag]").hide();
          setTimeout(function () {
            ...
            $("#rename input[name=tag]").val(JSON.stringify(gClient.ppl[gClient.participantId].tag));
          }, 100);
        } else if (id) {
      ...
      if (gClient.user.admin == true) {
        $('<div class="menu-item set-color">Set Color</div>').appendTo(menu)
          .on("mousedown touchstart", function (evt) {
            ...
            gClient.sendArray([{ m: "admin-userset", id: part._id, set:{color: color} }]);
          });
        $('<div class="menu-item set-name">Set Name</div>').appendTo(menu)
          .on("mousedown touchstart", function (evt) {
            var name = prompt("What name?", part.name);
            if (name === null) return;
            gClient.sendArray([{ m: "admin-userset", id: part._id, set:{name: name} }]);
          });
      }
      menu.fadeIn(100);
    };
				...
				function(midi) {
					...
					function midimessagehandler(evt) {
						...
						if(cmd == 8 || (cmd == 9 && vel == 0)) {
							// NOTE_OFF
							release(channel, MIDI_KEY_NAMES[note_number - 9 + MIDI_TRANSPOSE]);
						} else if(cmd == 9) {
							// NOTE_ON
							if(evt.target.volume !== undefined)
								vel *= evt.target.volume;
							press(channel, MIDI_KEY_NAMES[note_number - 9 + MIDI_TRANSPOSE], vel / 100);
						} else if(cmd == 11) {
							// CONTROL_CHANGE
							if(!gAutoSustain) {
								if(note_number == 64) {
					...
					function plug() {
						...
						if(midi.outputs.size > 0) {
							...
							gMidiOutTest = function(note_name, vel, delay_ms, channel) {
								// console.log(channel)
								...
								for(var output_it = outputs.next(); output_it && !output_it.done; output_it = outputs.next()) {
									...
									if(output.enabled) {
										...
										if(output.volume !== undefined)
											v *= output.volume;
										output.send([0x90 + channel, note_number, v], window.performance.now() + delay_ms);
									}
								}
							}
						}
...
function catSound() {
	let sounds = [ // add URLs here
		"https://multiplayerpiano.dev/cat-sounds/meow1.mp3",
		"https://multiplayerpiano.dev/cat-sounds/meow2.mp3",
		"https://multiplayerpiano.dev/cat-sounds/meow3.mp3",
		"https://multiplayerpiano.dev/cat-sounds/meow4.mp3",
		"https://multiplayerpiano.dev/cat-sounds/meow5.mp3",
		"https://multiplayerpiano.dev/cat-sounds/meow6.mp3",
		"https://multiplayerpiano.dev/cat-sounds/meow7.mp3",
		"https://multiplayerpiano.dev/cat-sounds/meow8.mp3",
		"https://multiplayerpiano.dev/cat-sounds/meow9.mp3",
		"https://multiplayerpiano.dev/cat-sounds/meow10.mp3"
	];
