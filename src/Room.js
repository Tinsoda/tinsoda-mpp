...
class Room extends EventEmitter {
    ...
    join(cl) { //this stuff is complicated
        ...
        if (!otheruser) {
            ...
            if (((this.connections.length == 0 && Array.from(this.ppl.values()).length == 0) && !this.isLobby(this._id)) || this.crown && this.crown.userId == undefined) { //user that created the room, give them the crown.
                ...
                this.crowndropped = false;
                this.settings = this.settings !== undefined ? this.settings : {visible:true,color:this.server.defaultRoomColor,chat:true,crownsolo:false};
            } else {
                //cl.quotas.a.setParams(Quota.PARAMS_A_NORMAL);
                this.settings = {visible:true,color:this.server.defaultRoomColor,chat:true,crownsolo:false,lobby:this.isLobby(this._id) ? true : false};
            }
